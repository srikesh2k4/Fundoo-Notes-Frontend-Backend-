...
builder.Services.AddScoped<INoteService, NoteService>(provider => new NoteService(
...
builder.Services.AddSwaggerGen(options =>
{
    options.SwaggerDoc(
        ...
        new OpenApiInfo
        {
            ...
            Version = "v1",
            Description =
                "A comprehensive note-taking application API with authentication, collaboration, and labeling features",
...
// Configuration Verification (Debug)
...
Console.WriteLine(
    $"CORS Origins: {(corsOrigins != null ? string.Join(", ", corsOrigins) : "None (Allow All)")}"
);
Console.WriteLine(new string('-', 60));
...
if (
    ...
    || builder.Configuration.GetValue<bool>("UseHttpsRedirection", false)
)
{
...
Console.WriteLine(
    $"Swagger UI: {(app.Environment.IsDevelopment() ? "https://localhost:7014" : "Not available in production")}\n"
);
